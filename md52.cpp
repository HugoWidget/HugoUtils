...
void MD5_Final(unsigned char digest[16], MD5_CTX* context) {
	...
	for (int i = 0; i < 8; i++) bits[i] = (unsigned char)((context->count[(i >= 4 ? 1 : 0)] >> ((i % 4) * 8)) & 0xff);
	index = (context->count[0] >> 3) & 0x3f;
...
std::string MD5_Hash(const std::string input) {
	...
	char buf[33] = {};
	MD5_Init(&context);
	MD5_Update(&context, reinterpret_cast<const unsigned char*>(input.c_str()), (unsigned int)input.length());
	MD5_Final(digest, &context);
	for (int i = 0; i < MD5_DIGEST_LENGTH; i++)
		sprintf_s(&buf[i * 2], 3, "%02x", (unsigned int)digest[i]);
	return buf;
}